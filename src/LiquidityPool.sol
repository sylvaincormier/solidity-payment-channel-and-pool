...
contract LiquidityPool is ReentrancyGuard {
    ...
    function _checkTradeDelay() private {
        require(block.number >= _lastTradeBlock[msg.sender] + MINIMUM_TRADE_DELAY, "Must wait before trading again");
        ...
        returns (uint256 amountA, uint256 amountB, uint256 liquidity)
    {
        ...
        if (_reserveA == 0 && _reserveB == 0) {
            ...
            liquidity =
                Math.min((amountA * lpToken.totalSupply()) / _reserveA, (amountB * lpToken.totalSupply()) / _reserveB);
        }
    ...
    function removeLiquidity(uint256 liquidity) external nonReentrant returns (uint256 amountA, uint256 amountB) {
        ...
        require((newK * 1e18) >= (oldK * ratio * ratio / 1e18), "K value check failed");
    ...
    function swap(uint256 amountIn, bool isAtoB) external nonReentrant returns (uint256 amountOut) {
        ...
        uint256 balanceBefore = isAtoB ? tokenA.balanceOf(address(this)) : tokenB.balanceOf(address(this));
        ...
        uint256 balanceAfter = isAtoB ? tokenA.balanceOf(address(this)) : tokenB.balanceOf(address(this));
